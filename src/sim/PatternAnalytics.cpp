...
namespace sim {
    ...
    namespace {
        ...
        inline int hour_of(const std::chrono::system_clock::time_point& ts) {
            using namespace std::chrono;
            auto sec = duration_cast<seconds>(ts.time_since_epoch()).count();
            auto hrs = sec / 3600;
            return static_cast<int>(hrs % 24);
        }

        // Helper to get day of month out of record
        inline int extract_day(const SensorRecord& r) {
            using namespace std::chrono;
            auto sec = duration_cast<seconds>(r.ts.time_since_epoch()).count();
        ...
        inline int extract_month(const SensorRecord& r) {
            using namespace std::chrono;
            auto sec = duration_cast<seconds>(r.ts.time_since_epoch()).count();
            std::time_t t = static_cast<std::time_t>(sec);
            std::tm tm = to_utc_tm(t);
    ...
    DailyFlags analyse_day(const std::vector<SensorRecord>& day,
    ...
    MonthlySummary analyse_month(const std::vector<SensorRecord>& month,
        const Criteria& criteria) {
        MonthlySummary ms{};
